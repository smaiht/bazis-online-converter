...
def normalize_panel_rotation(entity):
    ...
    # TODO: хуй знает почему, но тут нужно писать повороты в нашей системе координат но в порядке х-у-з (??? это не точно)
    # то есть если деталь стоит вертикально в нулевых поворотах, то какие повороты задать чтобы она стала такой как в про100
    # X - влево, Y - up, Z - на нас

    # Случай 5: (x,y,z) = (l,d,w) - Боковая панель, но повернута вокруг своей оси на 90 ---- это ведь лежачая, кейс с боковой не могу вспомнить
    if (abs(base_x - target_height) < eps and 
        abs(base_y - target_depth) < eps and 
        abs(base_z - target_width) < eps):
